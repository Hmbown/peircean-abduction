...
def check_python_version() -> bool:
    ...
    if version.major == 3 and version.minor >= 10:
        ...
        console.print(
            f"[red]❌ Python 3.10+ required. Found {version.major}.{version.minor}[/red]"
        )
        return False
...
def check_dependencies() -> None:
    ...
    for dep in dependencies:
        if importlib.util.find_spec(dep):
            ...
            console.print(f"[yellow]⚠️  Optional dependency '{dep}' not found (some features may be limited)[/yellow]")


def check_mcp_server() -> bool:
    """Check if MCP server can be imported and tools are registered."""
    try:
...
def check_claude_config() -> None:
    """Check if Claude Desktop config exists and has peircean configured."""
    ...
    try:
        ...
        try:
            ...
            if "peircean" in servers:
                ...
                console.print(f"[green]✅ Found 'peircean' in Claude config[/green]")
                console.print(f"   Command: [dim]{cmd} {' '.join(args)}[/dim]")
                
                # Verify python path matches current env
                if cmd != sys.executable:
                     console.print(f"[yellow]⚠️  Config uses different Python: {cmd}[/yellow]")
                     console.print(f"[dim]   Current Python: {sys.executable}[/dim]")
                     console.print("[yellow]   Run 'peircean install' to update config to current environment[/yellow]")
            else:
